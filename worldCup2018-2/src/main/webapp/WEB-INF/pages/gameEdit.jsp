	...
	$("#add-carton").click(function(e){
		...
		$("#score2").change(function(e){
			for(i=0;i < parseInt($(this).val());i++)
			{
				...
				{
					$("#playerBody").append("<tr id='tr-player-"+i+"'><td class='player-team1'></td><td></td><td class='passer-team1'></td><td class='minute-team1'></td><td class='player-team2'><select id='select-player"+i+"' name='scorerPlayers2["+i+"]'><c:forEach var='player' items='${game.team2.players}'><option value='${player.id}' label='${player.number} - ${player.firstName} ${player.name}' /></c:forEach><option value='' label='CSC' /></select></td><td><input type='checkbox' name='scorerPenalty2["+i+"]'/></td><td class='player-team2'><select id='select-passer"+i+"' name='passerPlayers2["+i+"]'><option value='' label='Aucun' /><c:forEach var='player' items='${game.team2.players}'><option value='${player.id}' label='${player.number} - ${player.firstName} ${player.name}' /></c:forEach></select></td><td class='minute-team2'><select id='select-minute"+i+"' name='scorerPlayersMinute2["+i+"]'><c:forEach begin='1' end='90' var='minute'><option value='${minute}' label='${minute}'/></c:forEach></select></td></tr>");
				}
			}
			if($('tr[id^="tr-player-"]').length > parseInt($(this).val()))
			{
		...
		$("#resultBody").on("change","select[name='scoreProlong1']",function(e){
			for(i=0;i < parseInt($(this).val());i++)
			{
				...
				{
					$("#playerBody").append("<tr id='tr-player-prolong-"+i+"'><td class='player-team1'><select id='select-player"+i+"' name='scorerPlayersProlong1["+i+"]'><c:forEach var='player' items='${game.team1.players}'><option value='${player.id}' label='${player.number} - ${player.firstName} ${player.name}' /></c:forEach><option value='' label='CSC' /></select></td><td><input type='checkbox' name='scorerProlongPenalty1["+i+"]'/></td><td class='player-team1'></td><td><select id='select-passer"+i+"' name='passerPlayersProlong1["+i+"]'><option value='' label='Aucun' /><c:forEach var='player' items='${game.team1.players}'><option value='${player.id}' label='${player.number} - ${player.firstName} ${player.name}' /></c:forEach></select></td><td class='minute-team1'><select id='select-minute"+i+"' name='scorerPlayersMinuteProlong1["+i+"]'><c:forEach begin='91' end='120' var='minute'><option value='${minute}' label='${minute}'/></c:forEach></select></td><td></td><td class='player-team2'></td><td class='passer-team2'></td><td class='minute-team2'></td></tr>");
				}
			}
